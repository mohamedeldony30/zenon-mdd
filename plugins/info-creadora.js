...
await conn.sendFile(m.chat, pp, 'lp.jpg', cat, fkontak, false, { contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: ' 😻 𝗦𝘂𝗽𝗲𝗿 ZenonBot-𝗠𝗗 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 ', previewType: 0, thumbnail: gataImg, sourceUrl: accountsgb.getRandom()}}})
...
//------- Nombre
  ...
  let insta = `https://www.instagram.com/f0.0n1`
  let teksnomor = `
• @${wm.split`@`[0]} •
------- ${wm} -------
`
    ...
    if (/(contacto|owner|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador)/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
         ...
         // conn.reply(m.chat, "Nombre del bot : zenon-md 🐈", m, { contextInfo: { mentionedJid: [nowner] }})
            break
            case 'bio':
